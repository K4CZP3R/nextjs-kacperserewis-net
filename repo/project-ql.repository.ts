...
import { IProject, PROJECT_DATA_GRAPHQL_CONTENT, toProject } from '../models/project.model';
...
export class ProjectQlRepository extends BaseQlRepository<IProject> {
  ...
  public async getBySlug(slug: string): Promise<IProject[] | null> {
    ...
    return data[this.modelNamePlural].data.map((raw: any) => this.rawConverter(raw));
  }
  ...
  public async getSlugs(): Promise<string[]> {
    ...
    return data[this.modelNamePlural].data.map((raw: any) => raw.attributes.slug);
  }
}
