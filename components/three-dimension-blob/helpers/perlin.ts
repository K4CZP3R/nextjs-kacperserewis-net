...
export class Perlin {
    ...
    private perm: Array<number> = new Array(512);
    private gradP: Array<Grad> = new Array(512);
    ...
    private init(seed: number) {
        ...
        for (let i = 0; i < 256; i++) {
            ...
            this.gradP[i] = this.gradP[i + 256] = Perlin.grad3[v % 12];
        }
    }
    ...
    perlin3(x: number, y: number, z: number) {
        ...
        return Perlin.lerp(
            Perlin.lerp(
                Perlin.lerp(n000, n100, u),
                Perlin.lerp(n001, n101, u), w),
            Perlin.lerp(
